...
// ---------- Geocode（建議版 C：先 region=tw 偏好；無結果再全域） ----------
async function geocodeGoogle(query: string): Promise<LatLng & { formatted: string }> {
  const key = process.env.GOOGLE_MAPS_API_KEY!;
  const base = 'https://maps.googleapis.com/maps/api/geocode/json';

  // A. 偏好台灣（不是硬限制）
  const urlTW = `${base}?address=${encodeURIComponent(query)}&language=zh-TW&region=tw&key=${key}`;
  let j = await fetchJson<any>(urlTW);

  // 若沒有結果，B. 退回全域
  if (!j.results?.[0]) {
    const urlGlobal = `${base}?address=${encodeURIComponent(query)}&language=zh-TW&key=${key}`;
    j = await fetchJson<any>(urlGlobal);
  }

  if (!j.results?.[0]) {
    throw new Error(j.error_message || 'geocode_failed');
  }
...
async function routeGoogle(origin: string, destination: string) {
  ...
  const url =
    ...
    `&language=zh-TW&region=tw&mode=driving&key=${key}`;

  const j = await fetchJson<any>(url);
  if (j.status !== 'OK' || !j.routes?.[0]) {
    throw new Error(j.error_message || j.status || 'directions_failed');
  }
...
async function geocodeOSM(query: string) {
  ...
  const r = await fetch(url, { headers: { 'Accept-Language': 'zh-TW' }, cache: 'no-store' });
