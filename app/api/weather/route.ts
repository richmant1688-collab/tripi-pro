...
const units = UNITS.has(searchParams.get('units') ?? '') ? (searchParams.get('units') as string) : 'metric';
const lang = (searchParams.get('lang') ?? 'zh_tw').toLowerCase();


const KEY = process.env.OPENWEATHER_API_KEY;
if (!KEY) return NextResponse.json({ error: { code: 'CONFIG', message: 'OPENWEATHER_API_KEY missing' } }, { status: 500 });
